//
//RandomaddForce.cs for unity-chan!
//
//Original Script is here:
//ricopin / RandomaddForce.cs
...
namespace UnityChan
{
	public class RandomaddForce : MonoBehaviour
	{
		...
		void Update ()
		{
			...
			if (isaddForceActive) {
				if(isMinus){
					force = new Vector3 (Mathf.PerlinNoise (Time.time, 0.0f) * addForcePower * -0.001f , gravity * -0.001f , 0);
				}else{
					force = new Vector3 (Mathf.PerlinNoise (Time.time, 0.0f) * addForcePower * 0.001f, gravity * -0.001f, 0);
				}
		...
		void OnGUI ()
		{
            if (isGUI)
            {
                ...
                isaddForceActive = GUI.Toggle(rect1, isaddForceActive, "Random addForce");
            }
		}
